...
investment_source_enum = sa.Enum("offering", "marketplace", name="investment_source")
...
def upgrade() -> None:
    ...
    op.add_column(
        "INVESTMENT", sa.Column("source", investment_source_enum, nullable=False)
    )
    op.add_column("INVESTMENT", sa.Column("is_active", sa.Boolean(), nullable=False))
    op.add_column(
        "INVESTMENT", sa.Column("tx_hash", sa.String(length=255), nullable=True)
    )
    op.create_foreign_key(
        op.f("fk_INVESTMENT_token_id_TOKEN"),
        "INVESTMENT",
        "TOKEN",
        ["token_id"],
        ["id"],
...
def downgrade() -> None:
    """Downgrade schema."""
    op.drop_constraint(
        op.f("fk_INVESTMENT_token_id_TOKEN"), "INVESTMENT", type_="foreignkey"
    )
