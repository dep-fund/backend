...
class ProjectAdvanceService:
    ...
    async def create(
        self, project_id: UUID, user_id: UUID, *, description: str, url: str
    ...
    async def _get_advance(self, project_id: UUID, number: int) -> ProjectAdvance:
        adv = await self.session.scalar(
            select(ProjectAdvance).where(
                ProjectAdvance.project_id == project_id, ProjectAdvance.number == number
            )
        )
        if not adv:
    ...
    async def list_by_project(self, project_id: UUID) -> list[ProjectAdvanceResponse]:
        ...
        result = await self.session.scalars(
            select(ProjectAdvance)
            .where(ProjectAdvance.project_id == project_id)
            .order_by(ProjectAdvance.number)
        )
        return [ProjectAdvanceResponse.model_validate(adv) for adv in result.all()]

    async def admin_list_by_project(
        self, project_id: UUID
    ) -> list[ProjectAdvanceResponse]:
        await self._project_service._get_project(project_id)
