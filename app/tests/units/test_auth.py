...
from app.exceptions.auth.auth import InvalidCredentials, PasswordResetTokenInvalid
...
async def test_login_success(
    MockTokenService, MockVerifyPassword, MockUserService, mock_session
):
    # Setup
    mock_user = MagicMock()
    mock_user.id = uuid4()
    ...
    mock_user_service_instance.get_with_role_and_permissions = AsyncMock(
...
async def test_login_invalid_credentials(
    MockVerifyPassword, MockUserService, mock_session
):
    # Setup
    mock_user_service_instance = MockUserService.return_value
    mock_user_service_instance.get_with_role_and_permissions = AsyncMock(
...
async def test_forgot_password_user_exists(
    MockMailService, MockTokenService, MockUserService, mock_session, background_tasks
):
    mock_user = MagicMock()
    mock_user.id = uuid4()
